...
import HouseTab.State (HouseTabEntry(..), Result(..),Date(..),Person(..))
...
getPercentage p d = fn (percs p)
    where
      fn (p:[]) = snd p
      fn (p:ps) = if fst (head ps) > d 
                    then snd p
                    else fn ps
...
purchasify people ((HouseTabEntry _ who _ when howmuch whopays):xs) = 
    (Purchase (getPerson people (head who))
              when
              howmuch 
...
doTheSplit purchase = map (\p -> (p, (ammount purchase) * (gp p) / (if unity == 0 then 1 else unity))) (payers purchase)
    where unity = sum (map (\p -> gp p) (payers purchase))
...
addMissing house (Result personResult date) = Result (personResult ++ (map (\p -> (p,0,0)) missing))
